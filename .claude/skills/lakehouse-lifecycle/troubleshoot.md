...
| "start fresh" leaves a broken/inconsistent stack | `./lakehouse doctor` | Never use `docker compose down -v` to reset (it can't touch host Postgres/SeaweedFS — see [stop.md](stop.md)). Use `./lakehouse reset --all` (or `--data`/`--metadata`); back up first with `./lakehouse backup` if state matters. `doctor` reports orphans + any interrupted reset/restore. |
